...
{ /// pointer to the left branch (a sub-tree)
  ...
  /// the data item (we use a long here)
  long data;                
}
tree_node;
...
static int count_leaves(tree_node *link)
{
  ...
  if (link->left!=NULL){
    count = count + count_leaves(link->left);
  }
  if (link->right!=NULL){
    count = count + count_leaves(link->right);
  }
  return count;
}
...
static int tree_height(tree_node *link)
{
  ...
  int tr=1;
  if(link->left!=NULL){
    tl = tl + tree_height(link->left);
  }
  if(link->right!=NULL){
    tr = tr + tree_height(link->right);
  }
  if (tr>=tl){
    return tr;
  }
...
static int count_function_calls_on_hit(tree_node *link,int level)
{
  ...
  if (link->left!=NULL){
    c = c + count_function_calls_on_hit(link->left,level+1);
  }
  if (link->right!=NULL){
    c = c + count_function_calls_on_hit(link->right,level+1);
  }
  return c;
}
...
static int count_function_calls_on_miss(tree_node *link,int level)
{
  ...
  if (link->left==NULL){
    ...
    c = c + count_function_calls_on_miss(link->left,level+1);
  }
  if (link->right==NULL){
    c= c + level+2;
  }
  else {
    c = c + count_function_calls_on_miss(link->right,level+1);
  }
  return c;
}
...
int main(int argc,char **argv)
{
  ...
  fprintf(f,"                                  data for %d random trees\n",n_experiments);
  fprintf(f,"          maximum tree height           number of leaves          calls on hit    calls on miss\n");
  fprintf(f,"        ----------------------  -------------------------------  --------------  --------------\n");
  fprintf(f,"     n  min max    mean    std    min   max       mean      std     mean    std     mean    std\n");
  fprintf(f,"------  --- --- ------- ------  ----- ----- ---------- --------  ------- ------  ------- ------\n");
  ...
  for(int n_log = 1 * 10;n_log <= 4 * 10;n_log++)
  {
    ...
    fprintf(f,"  %3d %3d %7.4f %6.4f",m,M,mean,std);
